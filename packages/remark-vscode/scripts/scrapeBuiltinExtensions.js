...
glob(
  path.resolve(__dirname, '../vscode/extensions/*/package.json'),
  async (err, packages) => {
    try {
      if (err) throw err
      ...
      // Copy langauges
      const grammarManifest = await extensions.reduce(
        async (newManifestPromise, extension) => {
          return {
            ...(await newManifestPromise),
            ...(await Object.keys(extension.grammars).reduce(
              async (newManifestPromise, scopeName) => {
      ...
      // Copy themes
      const themeManifest = await extensions.reduce(
        async (newManifestPromise, extension) => {
          return {
            ...(await newManifestPromise),
            ...(await Object.keys(extension.themes).reduce(
              async (newManifestPromise, themeId) => {
...
function processTmJson(content) {
  ...
  if (
    content.scopeName === 'source.shell' &&
    content.repository &&
    content.repository.heredoc
  ) {
    return {
      ...content,
      repository: {
