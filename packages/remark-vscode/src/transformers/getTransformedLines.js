...
export default async function getTransformedLines(
  transformers,
  text,
  languageName,
  meta
  ...
  linesLoop: for (let lineIndex = 0; lineIndex < rawLines.length; lineIndex++) {
    ...
    for (let i = 0; i < transformers.length; i++) {
      ...
      if (txResult.gutterCells) {
        gutterCellsPerTransformer[i] = Math.max(
          txResult.gutterCells.length,
          gutterCellsPerTransformer[i] || 0
        )
    ...
    result.push({
      ...
      setContainerClassName:
        joinClassNames(...addedContainerClassNames) || undefined,
    })
  }

  const flattenedGutterCells = flattenGutterCells(
    gutterCells,
    gutterCellsPerTransformer
  )
  return result.map((line, i) => ({
    ...line,
    gutterCells: flattenedGutterCells[i],
  }))
}
    ...
    for (let i = 0; i < transformerResults.length; i++) {
      ...
      if (currentTransformerCells) {
        for (let j = 0; j < currentTransformerCells.length; j++) {
          result[(gutterCellsPerTransformer[i - 1] || 0) + j] =
            currentTransformerCells[j]
        }
      }
    }
    return result
