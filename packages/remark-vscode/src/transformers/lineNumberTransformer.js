import { getCommentContent, getCommentRegExp } from './transformerUtils.js'
import { getScope } from '../storeUtils.js'
const lineNumberRegExp = /L(\d+)$/

export function createLineNumberLineTransformer(languageAliases, cache) {
  let grammarCache
  ...
  const lineNumberTransformer = async ({ meta, state, line, language }) => {
    ...
    if (meta.numberLines !== undefined) {
      const lineNumber =
        typeof meta.numberLines === 'number' ? meta.numberLines : 1
      return {
        state: lineNumber,
        line,
        gutterCells: [
