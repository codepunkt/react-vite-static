...
const declarations = createNamespaceDeclaration(
  gatherTypeDeclarations(gatherEnums())
)
        ...
        types.push(
          ts.createInterfaceDeclaration(
            ...
            node.interfaces.length
              ? [transformImplementsToExtends(node.interfaces)]
              : undefined,
    ...
    return ts.createHeritageClause(
      ...
      interfaces.map(({ name }) =>
        ts.createExpressionWithTypeArguments(
          undefined,
          ts.createIdentifier(name.value)
        )
      )
    )
  }
      ...
      case 'NamedType': {
        ...
        return enumType
          ...
          : ts.createTypeReferenceNode(
              mapTypeName(typeNode.name.value),
              undefined
            )
      }
    }
